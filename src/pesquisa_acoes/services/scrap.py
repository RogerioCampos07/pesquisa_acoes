...
class TickerScraper:
    ...
    def search_ticker(self, ticker):
        try:
            ...
            try:
                get_info_ticker = self.driver.find_element(
                    By.XPATH, "//h1[@class='yf-4vbjci']"
                )
                get_price_ticker = self.driver.find_element(
                    By.XPATH, "//li[span[@title='Bid']]/span[@class='value yf-1b7pzha']"
                )
